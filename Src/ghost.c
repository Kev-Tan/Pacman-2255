...
Ghost* ghost_create(int flag) {
	...
	case Pinky:
		...
		ghost->move_script = &ghost_random_move_script;
		break;
	case Inky:
		ghost->objData.Coord.x = cage_grid_x;
		ghost->objData.Coord.y = cage_grid_y;
...
void ghost_draw(Ghost* ghost) {
	...
	*/
	...
	else if (ghost->objData.facing == DOWN) {
		if (ghost->objData.moveCD <= 32)
			al_draw_scaled_bitmap(ghost->move_sprite, 96, 0,
				16, 16,
				drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
...
void ghost_collided(Ghost* ghost) {
	if (ghost->status == FLEE || ghost->status == BLINK) {
		ghost->status = GO_IN;
		ghost->speed = 4;
	}
}
...
void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	if (shortestDirection == UP) {
		count = 0;
		for (Directions i = 1; i <= 4; i++) {
			if (ghost_movable(ghost, M, i, true) && i != 1) {
				directions[count++] = i;
			}
		}
		ghost_NextMove(ghost, directions[generateRandomNumber(0, count)]);
	}

	else if (shortestDirection == LEFT) {
		count = 0;
		for (Directions i = 1; i <= 4; i++) {
			if (ghost_movable(ghost, M, i, true) && i != 2) {
				directions[count++] = i;
			}
		}
		ghost_NextMove(ghost, directions[generateRandomNumber(0, count)]);
	...
	else if (shortestDirection == DOWN) {
		count = 0;
		for (Directions i =1; i <= 4; i++) {
			if (ghost_movable(ghost, M, i, true) && i != 4) {
				directions[count++] = i;
			}
		}
		ghost_NextMove(ghost, directions[generateRandomNumber(0, count)]);
	...
	else if (shortestDirection == RIGHT) {
		count = 0;
		for (Directions i = 1; i <= 4; i++) {
			if (ghost_movable(ghost, M, i, true) && i != 3) {
				directions[count++] = i;
			}
		}
	}
