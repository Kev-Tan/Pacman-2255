...
Map* create_map(const char* filepath) {
	...
	M->map = malloc(sizeof(char*) * (M->row_num));
	if (!M->map) {
		game_abort(stderr, "map char array malloc error\n");
		return NULL;
	}
	for (int i = 0; i < M->row_num; i++) {
		M->map[i] = malloc(sizeof(char**) * (M->col_num));
		if (!M->map[i]) {
			game_abort(stderr, "map char array malloc error\n");
			return NULL;
		}
...
static void draw_speed_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(25, 138, 55));
}
